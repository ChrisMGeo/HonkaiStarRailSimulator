...
var butTheBattleIsntOver = new ButTheBattleIsntOver();
bronya.Lightcone = Some<Lightcone>.Of(butTheBattleIsntOver);
{
    ...
    newBronyaSpeed.FlatBonus += 25.032f + 7.2f + 4f + 4f + 7.2f + 7.2f + .3f * 5;
    bronya.Speed =
        newBronyaSpeed; // Setters only work when reference is changed not just value, might work if I use INotifyPropertyChanged
    bronya.MaxHp.FlatBonus += 705.6f + 33.87f + 38.103755f + 42.33751f;
    bronya.MaxHp.PercentageBonus += (8.208f + 12.528f) / 100 + .432f;
    bronya.Def.FlatBonus += 52.921876999999995f;
    bronya.Def.PercentageBonus += (4.86f + 5.4f) / 100;
    bronya.Atk.FlatBonus += 352.8f + 40.220631f + 21.168754f;
    bronya.Atk.PercentageBonus += (7.776f + 4.32f + 12.528f) / 100;
}
...
guinaifen.Lightcone = Some<Lightcone>.Of(goodNightAndSleepWell);
{
    ...
    guinaifen.Atk.AddStatusEffect(new ConditionalStatusEffect(StatusEffectId.PermanentStatBuff,
        () => new StatModifier(percentageBonus: .25f * guinaifen.EffectHitRate.GetFinalValue())));
